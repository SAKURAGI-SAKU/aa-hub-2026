...
app.post("/api/register", async (req, res) => {
    if (await isBanned(req)) return res.status(403).json({ error: "お使いの環境からのアクセスは制限されています" });
...
app.post("/api/login", async (req, res) => {
    if (await isBanned(req)) return res.status(403).json({ error: "アクセス制限中です" });
...
app.post("/api/admin/ban", async (req, res) => {
    ...
    if (targetUser.value) {
        ...
        // IPをBANリストに追加
        if (ip && ip !== "unknown") {
            await kv.set(["banned_ips", ip], { bannedAt: new Date(), reason: "違反行為" });
        }
        // ユーザーアカウントを削除
        await kv.delete(["users", targetUserId]);
        res.json({ success: true, message: "ユーザーを追放し、IPをブロックしました" });
    } else {
...
// --- 以降、投稿・いいね・通報・通知・削除の既存APIをそのまま維持 ---
...
app.post("/api/posts/:id/like", async (req, res) => {
    ...
    if (post.value) {
        ...
        updated.likedBy.push(req.body.userId);
